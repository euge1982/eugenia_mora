...
<template>
    ...
    <div class= "list-wrapper max-w rounded overflow-y-auto shadow-lg mt-10 p-4 transition ease-linear" v-bind:class="reactiveTheme.isDark ? 'dark' : ''">
        ...
        <div v-for="(task, index) in reactiveTaskStore.data" :key="index" class="wrapper relative group border-black my-2 transition ease-linear">
            <form v-on:submit.prevent>
                ...
                <div class="btns absolute right-0 top-0 py-2 sm:py-2.5 px-2 w-20 h-14 flex justify-around cursor-default" >
                    ...
                    <button @click="reactiveTaskStore.removeTask(task)" class="p-1 cursor-pointer"><TrashIcon class="w-6 h-6 hover:text-red-500 "/></button>
                </div>
